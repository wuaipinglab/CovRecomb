...
def test_ld_genr(seed_gen, seed_gene_homo_rate, parallel_prop, seed_mut_perlin, turns_list, generations_list, temp_file, code_path, qmatrix, sample_rate, len_UAB = 4, seq_length = 29903, mut_rate = float(8E-4/52), rec_rate = 0.027):
    rng = default_rng()
    result_record = {}
    run_total_number = 0
    ...
    for genera in generations_list:
        ...
        for turns in range(1,turns_list+1):
            ...
            for generation in range(1, genera + 1):
                all_pos = [x for x in range(0,seq_length)]
                import random
                homo_mut = random.sample(all_pos, 20)
                seq_homo = ""
                for site in all_pos:
                    if site in homo_mut:
                        seq_homo += choose_new_letter(ref_seq["reference"][site])
                ...
                ## Recombination
                poisson = rng.poisson(rec_rate, len(pop))#len(pop))
            ...
            ## CovRecomb to detect recombinants
            feature_mutation = []
            for l in linA_list:
                for v in lin_fv75[l]:
...
turns_list = 10
sample_rate = 10
seed_gene_homo_rate = 0.2
parallel_prop = 0.1
seq_length = 29903
rec_rate = 0.1
mut_rate = float(8E-4/6)

column_names = ["seed_gen","generations_list","sampled_numer","turns_list","sample_rate","seed_gene_homo_rate","parallel_prop","seed_mut_perlin","UAB","lin_diff_num","mean_lin_diff","FPR","FDR","TPR"]
with open(temp_file+"ROC_results.txt","a+") as f:
            f.write(",".join(column_names)+"\n")

for len_UAB in [1,2,3,4,5]:
    for seed_mut_perlin in [3,5,8,10,15,20,30,40,50,60,70,80,90,100,130,150,200]:
        lin_diff_num,sampled_numer,mean_lin_diff,FPR,FDR,TPR = test_ld_genr(seed_gen, seed_gene_homo_rate, parallel_prop,seed_mut_perlin, turns_list,generations_list,temp_file,code_path,qmatrix,sample_rate,len_UAB,seq_length,mut_rate,rec_rate)
        with open(temp_file+"ROC_results.txt","a+") as f:
            ROC_results = [str(seed_gen),str(generations_list),sampled_numer,str(turns_list),str(sample_rate),str(seed_gene_homo_rate),str(parallel_prop),str(seed_mut_perlin),str(len_UAB),str(lin_diff_num),str(mean_lin_diff),FPR,FDR,TPR]

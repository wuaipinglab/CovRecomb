...
Parameters: (1) "-ld": The number of differential mutations between simulated lineages in the format of a list; Default: [46].
			...
			(3) "-sd","--seed_number": The number of seed (initial lineages; Default: 5.
			(4) "-turns","--turns_number": The number of independent simulation trials; Default: 10.
			(5) "-threshold","--least_number_of_sequential_feature_mutations": The threthold for the least_number_of_sequential_feature_mutation; Default: 4.
			(6) "-seqlength","--seq_length": The length of ATCG for each simulated sequence; Default: 29903.
			(7) "-mt","--mutation_rate": The nucleotide mutation rate for each generation; Default: float(8E-4/52).
			(8) "-rt","--recombination_rate": The recombination rate for each generation; Default: 0.1.
...
def choose_new_letter(letter):
    ...
    import random
    new_letter = random.choice(list(possible_letters.difference(set(letter))))
...
def generate_ref_and_lin(seq_length, lineage_num, lin_diff_num):
    ...
    all_pop = {}
    mut_perlin = lin_diff_num  #int(seq_length/lineage_num)

    all_pos = [x for x in range(0, seq_length)]
    # generate the seed sequence for each lineage
    already_pos = []
...
##############
def test_ld_genr(num_seeds, lin_diff_num_list, turns_list, generations_list, temp_file, code_path, qmatrix, len_UAB=4, seq_length=29903, mut_rate=float(8E-4 / 52), rec_rate=0.1):
    ...
    for lin_diff_num in lin_diff_num_list:
        ...
        for genera in generations_list:
            ...
            for turns in range(0, turns_list):
                ...
                print("lin_diff_num:", lin_diff_num, "  generation:", genera, "turns:", turns)
                ...
                for generation in range(1, genera + 1):
                    ...
                    for lin in pool:
                        if "bk" not in lin:
                            ...
                            if num_mut > 0:
                                import random
                                positions = random.sample(range(0, seq_length), num_mut) # The positions need to mutate
                ...
                seq_samples = copy.deepcopy(pop)
                # Extract feature mutations for each lineage 
                lin_fv75 = {}
                for num in range(1, num_seeds + 1):
                    ...
                    for s in seq_samples:
                        if "bk" not in s:
                            if (num == 1 and ("lin1" in s) and ("lin10" not in s) and ("bk" not in s)) or (num != 1 and ("lin" + str(num) in s) and ("bk" not in s)):  #单纯父母样本
                                count_target_lin += 1
                                # feature mutations
                ...
                if child_correct_AB_num == child_false_AB_num == 0:
                    turns_list += 1
                    continue
                else:
                    genera_number += 1
                    ...
                    df_result_record.loc[genera_number] = result_record[tagRun]
            ...
            df_cor_rate_record.loc[run_total_number] = [lin_diff_num,genera,str(turns+1),mean(rr_geneer_yang_cr),\
                mean(rr_geneer_yin_cr),mean(x_FPR_turns_mean),mean(y_TPR_turns_mean)]
...
def main(sysargs=sys.argv[1:]):
    parser = argparse.ArgumentParser(
        ...
        usage='''python3 Simulator_CovRecombTest.py -ld [15,21,38,46] -gen [4]''')
    ...
    parser.add_argument("-cp", "--code_path", help="The file address of the folder with scripts", default=os.getcwd() + "/")
    ...
    for i in range(1, cor_num + 1):
        globals()["p" + str(i)] = Process(target=test_ld_genr, args=(num_seeds, [lin_diff_num_list[i - 1]], turns_list, generations_list, temp_file, code_path, qmatrix, len_UAB, seq_length, mut_rate, rec_rate))
